                ...
                <div className="relative" ref={drilldownDropdownRef}>
                  ...
                  {drilldownDropdownOpen && (
                    <div className="absolute top-full left-0 mt-1 bg-white rounded-lg shadow-lg border border-gray-200 py-1 min-w-[140px] z-50">
                      ...
                      <button 
                        onClick={() => { setViewMode('DRILLDOWN'); setDrilldownSubView('companies'); setDrilldownDropdownOpen(false); }}
                        className={`w-full flex items-center gap-2 px-3 py-2 text-sm text-left hover:bg-gray-50 ${viewMode === 'DRILLDOWN' && drilldownSubView === 'companies' ? 'text-blue-600 bg-blue-50' : 'text-gray-700'}`}
                      >
                        <Building2 size={16} />
                        Companies
                      </button>
