...
export const getAccountHierarchy = async (accountId: string, activeOnly: boolean = false, depth: 'summary' | 'full' = 'full', signal?: AbortSignal): Promise<any> => {
...
export const buildAccountHierarchyFromApi = async (accountId: string, activeOnly: boolean = false, signal?: AbortSignal): Promise<AccountStructure | null> => {
  try {
    console.log(`Fetching hierarchy for account: ${accountId} (activeOnly: ${activeOnly})`);
    const rawData = await getAccountHierarchy(accountId, activeOnly, 'full', signal);
